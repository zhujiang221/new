<template>
  <div class="diagnosis-list-doctor-modern">
    <!-- 页面标题 -->
    <div class="modern-page-header">
      ...
      <p class="modern-page-subtitle">查看和管理宠物的诊断记录</p>
    </div>
...
import { getUserInfo, ROLE_DOCTOR } from '../../utils/user';
...
const isDoctor = computed(() => userInfo?.role === ROLE_DOCTOR);
...
async function fetchData() {
  ...
  try {
    ...
    const resp = await http.get('/user/diagnosis/getAllByLimitDoctor', { params });
...
function handlePrescribe(item: Diagnosis) {
  router.push({
    path: '/doctor/prescribe-medicine',
    query: { diagnosisId: item.id }
  });
}

async function handleDelete(item: Diagnosis) {
  const confirmed = await showConfirm('确认删除该记录吗？');
  if (!confirmed) return;
  
  try {
    const resp = await http.post('/user/diagnosis/del', { id: item.id });
...
.diagnosis-card-actions .modern-btn {
  flex: 1;
  min-width: 120px;
}
