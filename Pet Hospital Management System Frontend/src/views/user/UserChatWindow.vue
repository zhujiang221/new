...
function handleWebSocketMessage(message: WebSocketMessage) {
  console.log('[UserChatWindow] 收到WebSocket消息:', message);
  ...
  if (message.type === 'chat') {
    ...
    if (message.data) {
      ...
      if (message.data.type === 'chat' && message.data.data) {
        const rawMessage = message.data.data;
        // 使用类型守卫验证消息格式
        if (isChatMessage(rawMessage)) {
          chatMessage = rawMessage;
          console.log('[UserChatWindow] 从嵌套结构中提取聊天消息:', chatMessage);
        } else {
          console.warn('[UserChatWindow] 嵌套消息格式验证失败:', rawMessage);
        }
      } else if (message.data.sessionId !== undefined) {
        // 直接是ChatMessage对象，使用类型守卫验证
        if (isChatMessage(message.data)) {
          chatMessage = message.data;
          console.log('[UserChatWindow] 直接使用message.data作为聊天消息:', chatMessage);
        } else {
          console.warn('[UserChatWindow] 消息格式验证失败:', message.data);
        }
      }
    }
    
    if (chatMessage && sessionId.value) {
      // 使用全局状态管理添加消息
      chatStore.addMessage(sessionId.value, chatMessage);
      
      // 检查消息是否属于当前会话
      if (chatMessage.sessionId === sessionId.value) {
        console.log('[UserChatWindow] 消息属于当前会话，会话ID:', sessionId.value);
        
        // 检查是否是撤回消息
        if (chatMessage.isRevoked === 1) {
          chatStore.updateMessage(sessionId.value, chatMessage.id!, { isRevoked: 1 });
          console.log('[UserChatWindow] 消息已撤回，ID:', chatMessage.id);
        } else {
          // 滚动到底部
          nextTick(() => {
            scrollToBottom();
          ...
          if (chatMessage.receiverId === Number(currentUserId.value)) {
            markChatMessageRead(sessionId.value).then(() => {
              // 标记已读后，触发刷新未读数量的事件
              window.dispatchEvent(new CustomEvent('chat-message-read', { detail: { sessionId: sessionId.value } }));
            }).catch(() => {});
          }
        }
      } else {
        console.log('[UserChatWindow] 消息不属于当前会话，当前会话ID:', sessionId.value, '消息会话ID:', chatMessage.sessionId);
      }
    } else {
      console.warn('[UserChatWindow] 无法提取聊天消息，message.data:', message.data);
    }
  }
}
...
async function loadSession() {
  ...
  try {
    ...
    // 设置对方（医生）信息
    otherUserInfo.value = {
      name: session.doctorName || '医生',
      img: session.doctorImg
    };
  } catch (e) {
...
async function loadMessages() {
  ...
  loading.value = true;
  try {
    console.log('[UserChatWindow] 加载消息，会话ID:', sessionId.value);
    ...
    const validMessages = (list || []).filter(msg => {
      if (isChatMessage(msg)) {
        ...
        console.warn('[UserChatWindow] 无效的消息格式，已过滤:', msg);
        return false;
      }
    });
    ...
    // 标记已读后，触发刷新未读数量的事件
    window.dispatchEvent(new CustomEvent('chat-message-read', { detail: { sessionId: sessionId.value } }));
    console.log('[UserChatWindow] 消息加载完成，消息数:', validMessages.length);
  } catch (e) {
    console.error('[UserChatWindow] 加载消息失败:', e);
  } finally {
    loading.value = false;
  }
}
...
async function sendMessage() {
  ...
  try {
    console.log('[UserChatWindow] 发送消息，会话ID:', sessionId.value, '内容:', text);
    ...
    if (isChatMessage(sentMessage)) {
      ...
      await nextTick();
      scrollToBottom();
      console.log('[UserChatWindow] 消息已发送并添加到列表，消息ID:', sentMessage.id);
    } else {
      console.warn('[UserChatWindow] 返回的消息格式无效:', sentMessage);
    }
    ...
    console.error('[UserChatWindow] 发送消息失败:', e);
...
async function handleImageSelect(event: Event) {
  ...
  try {
    console.log('[UserChatWindow] 上传图片，会话ID:', sessionId.value);
    ...
    if (isChatMessage(sentMessage)) {
      ...
      await nextTick();
      scrollToBottom();
      console.log('[UserChatWindow] 图片已发送并添加到列表，消息ID:', sentMessage.id);
    } else {
      console.warn('[UserChatWindow] 返回的消息格式无效:', sentMessage);
    }
