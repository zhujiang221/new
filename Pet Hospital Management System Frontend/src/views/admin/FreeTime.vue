  ...
  <div class="free-time-modern">
    ...
    <div v-if="!selectedDay" class="calendar-view">
      <div class="modern-page-header">
        ...
        <p class="modern-page-subtitle">点击日期查看当天所有医生的可预约时间</p>
      </div>
    ...
    <div v-else class="day-detail-view">
      ...
      <div v-else-if="filteredDoctorTimeSlots.length === 0" class="modern-empty-state">
        <div class="modern-empty-state-icon">👨‍⚕️</div>
        <div class="modern-empty-state-text">
          {{ searchDoctorName || selectedAppointmentType ? '未找到符合条件的医生' : '暂无医生数据' }}
        </div>
      </div>
      <div v-else class="doctors-container">
        <div
          v-for="doctor in filteredDoctorTimeSlots"
          :key="doctor.id"
        ...
        >
          <div class="doctor-header">
            ...
            <div class="doctor-info">
              ...
              <div class="doctor-services">
                ...
                <span v-if="!doctor.services || doctor.services.length === 0" class="no-services">
                  暂无服务类型
                </span>
              </div>
            </div>
          ...
          <div class="time-slots-container">
            <div v-if="doctor.loading" class="time-slot-loading">加载中...</div>
            <div v-else-if="doctor.timeSlots.length === 0" class="no-time-slots">
              该医生当天无排班
            </div>
...
const doctorList = ref<{ id: string; name: string }[]>([]);
const doctorTimeSlots = ref<Doctor[]>([]);
const currentDate = ref(new Date());
const searchDoctorName = ref('');
const selectedAppointmentType = ref('');
const allAppointmentTypes = ref<{ id: string; name: string }[]>([]);
...
const filteredDoctorTimeSlots = computed(() => {
  return doctorTimeSlots.value.filter((doctor) => {
    const matchName = searchDoctorName.value
      ? doctor.name.includes(searchDoctorName.value.trim())
      : true;

    const matchType = selectedAppointmentType.value
      ? doctor.services && doctor.services.some(s => s.id === selectedAppointmentType.value)
      : true;
...
async function fetchDoctors() {
  try {
    ...
    if (Array.isArray(data)) {
      doctors = data;
    } else if (data && typeof data === 'object' && data.rows) {
      doctors = data.rows;
    }
...
async function fetchDayDetails(date: string) {
  loading.value = true;
  doctorTimeSlots.value = doctorList.value.map(doctor => ({
    ...doctor,
    loading: true,
    timeSlots: [],
    services: []
  ...
  // 并行获取所有医生的时间段和服务类型
  const promises = doctorList.value.map(async (doctor) => {
    try {
      ...
      if (Array.isArray(timeSlotsResp.data)) {
        timeSlots = timeSlotsResp.data;
      } else if (timeSlotsResp.data && typeof timeSlotsResp.data === 'object' && timeSlotsResp.data.timeSlots) {
        timeSlots = timeSlotsResp.data.timeSlots;
      }
      ...
      console.error(`获取医生 ${doctor.name} 的数据失败:`, e);
      return {
        id: doctor.id,
        name: doctor.name,
        loading: false,
