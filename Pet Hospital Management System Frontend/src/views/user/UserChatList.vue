<template>
  <div class="user-chat-list">
    ...
    <div v-else-if="sessionList.length === 0" class="empty-state">
      ...
      <button class="btn btn-primary" @click="goToRequest">发起聊天申请</button>
    </div>
    <div v-else class="session-list">
      ...
      >
        <div class="session-avatar">
          <img v-if="getDoctorImg(session)" :src="getDoctorImg(session)" :alt="getDoctorName(session)" />
          <div v-else class="avatar-placeholder">{{ getDoctorName(session)?.charAt(0) || 'D' }}</div>
        </div>
        <div class="session-info">
          <div class="session-header">
            <div class="session-name">{{ getDoctorName(session) }}</div>
...
async function fetchSessions() {
  ...
  try {
    ...
    if (sessionList.value.length > 0) {
      console.log('第一个会话的医生信息:', {
        doctorName: sessionList.value[0].doctorName,
        doctorImg: sessionList.value[0].doctorImg,
        doctorId: sessionList.value[0].doctorId
      });
    }
  } catch (e) {
...
function getDoctorName(session: ChatSession): string {
  // 尝试多种可能的字段名
  return (session as any).doctorName || 
         (session as any).doctor_name || 
         (session as any).doctor?.name ||
         '未知医生';
}

function getDoctorImg(session: ChatSession): string | undefined {
  // 尝试多种可能的字段名
  return (session as any).doctorImg || 
         (session as any).doctor_img || 
         (session as any).doctor?.img;
}
...
function openChat(session: ChatSession) {
  ...
  // 确保在移动端也能正确跳转
  router.push(`/user/chat/${session.id}`).catch((err) => {
    // 忽略导航重复的错误
