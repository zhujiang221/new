<template>
  <div class="assess-modern">
    <div class="modern-page-header">
      ...
      <p class="modern-page-subtitle">基于最新日志与标准，查看多宠物雷达评估</p>
    </div>
    
    <div class="chart-card modern-card">
      <div class="chart-card-header">
        <div class="chart-title">雷达图评估</div>
        <div class="chart-tip">同时展示多只宠物最新体征</div>
      </div>
...
async function fetchData() {
  ...
  try {
    // Fetch pets
    const petsResp = await http.get('/user/pet/getAllByLimit', {
      params: { page: 1, limit: 100 }
    });
    const pets: Pet[] = petsResp.data.rows || [];
    ...
    // Fetch pet daily logs for each pet
    const dailyResp = await http.get('/user/petDaily/getAllByLimit', {
    ...
    pets.forEach((pet) => {
      ...
      // Get latest daily log for this pet（按时间倒序取最新一条）
      ...
      // Get standard for this pet type
    ...
    // 统一设置全局刻度上限，既考虑数据本身也考虑标准，避免数据超出雷达图
...
function renderChart(names: string[], radarData: any[], seriesData: any[], petCount: number) {
  ...
  // 根据宠物数量动态调整画布宽度，配合外层横向滚动，避免图表被挤出
  const baseWidth = 900; // 基础宽度
  const widthPerPet = 350; // 每只宠物额外宽度更大一些，避免挤压
